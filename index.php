...
function printCliHelp() {
	echo "Example usage: php index.php programPath testpubDir [-t timeLimit] [-s]
...
function printResult(array $status) {
	...
	echo (IS_CLI ? "" : "&nbsp;") . "
" . (IS_CLI ? "" : "<div><div style=\"background-color:#" . ($status['solution_ok'] && $status['time_ok'] ? "00a000" : "fa0000") . "/*#cfcfcf*/; text-align:center;\"><span style=\"color:white;\">") . "PUBLIC DATA INSTANCE: " . $status['task'] . ", result: " . (IS_CLI ? ($status['solution_ok'] && $status['time_ok'] ? "\033[32m" : "\033[31m") : "") . ($status['solution_ok'] ? "" : "IN") . "CORRECT SOLUTION" . (IS_CLI ? "\033[0m" : "</span></div></div>" ) . "
